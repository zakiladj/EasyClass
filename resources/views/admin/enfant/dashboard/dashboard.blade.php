    ...
    <div class="col-12">
      <div class="box">
        ...
        <div class="mb-15 d-flex align-items-center">
                <a href="{{ route('chart.js', ['preset' => 'today']) }}" class="btn btn-warning">اليوم</a>
                <a href="{{ route('chart.js', ['preset' => 'week']) }}" class="btn btn-primary ml-5">هذا الأسبوع</a>
                <a href="{{ route('chart.js', ['preset' => 'month']) }}" class="btn btn-info ml-5">هذا الشهر</a>
                <a href="{{ route('chart.js', ['preset' => 'year']) }}" class="btn btn-success ml-5">هذه السنة</a>

                {{-- يدز زر الطباعة لليمين --}}
                <a href="{{ route('chart.print', request()->query()) }}"
          ...
          {{-- اختيار بين تاريخين --}}
          <form method="GET" id="rangeForm" action="{{ route('chart.js') }}"  class="row g-2 mb-20 no-validate">
...
<section class="content">
  <div class="row">
    <div class="col-12">
      <div class="box">
        <div class="box-header with-border">
          <h4 class="box-title">تحليل: مجموع مدفوعات الأطفال يوميًا</h4>
          <p class="mb-0">من {{ $from }} إلى {{ $to }}</p>
        </div>
        <div class="box-body">
        <div class="mb-15 d-flex align-items-center">
          ...
          <div id="childDailyChart" style="min-height: 320px;"></div>
        </div>
      </div>
    </div>
  </div>
        ...
        dataLabels: {
            ...
            xaxis: {
             ...
             yaxis: {
                ...
                // yaxis: {
                //     title: { text: 'المبلغ (DZD)', style: { color: '#ffffff' } },
                //     labels: {
                //         style: { colors: '#ffffff' }  // لون أرقام المحور Y
                //     }
                // },

                // xaxis: {
                //     title: { text:'التواريخ', style: { color: '#ffffff' } },
                //     labels: {
                //         style: { colors: '#ffffff' }  // لون أرقام المحور Y
                //     }
                // },
            ...
            const childEl = document.querySelector("#childDailyChart");
            if (childEl) {
                 const preset = @json($preset);
                const chartType = (preset === 'today') ? 'bar' : 'line';
                const childLabels = @json($childLabels);
                const childValues = @json($childValues);

                new ApexCharts(childEl, {
                chart: { type: chartType, height: 320, toolbar: { show: true } },
                stroke: { curve: 'smooth', width: 3 },
                series: [{ name: 'مدفوعات الأطفال', data: childValues }],
                colors: ['#77B6EA', '#545454'],
                dataLabels: {
                    enabled: true,
                    formatter: (val) => val + ' DA',
             ...
             yaxis: {
                ...
                stroke: { curve: 'smooth' },
                title: { text: 'مجموع مدفوعات  ', align: 'left', style: { color: '#ffffff' } },
                markers: { size: 4, colors: ['#FFA41B'], strokeColors: '#fff', strokeWidth: 2, hover: { size: 7 } },
                grid: { borderColor: '#f1f1f1' },
