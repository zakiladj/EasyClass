    ...
    <div class="col-12">
      <div class="box">
        <div class="box-header with-border">
          <h4 class="box-title">تحليل: المصاريف مقابل الإيرادات يوميًا</h4>
          <p class="mb-0">من {{ $from }} إلى {{ $to }}</p>
          {{-- <h3> {{ $data }} </h3> --}}
        </div>
        <div class="box-body">
        <div class="mb-15 d-flex align-items-center">
                <a href="{{ route('charge.chart.js', ['preset' => 'today']) }}" class="btn btn-warning">اليوم</a>
                <a href="{{ route('charge.chart.js', ['preset' => 'week']) }}" class="btn btn-primary ml-5">هذا الأسبوع</a>
                <a href="{{ route('charge.chart.js', ['preset' => 'month']) }}" class="btn btn-info ml-5">هذا الشهر</a>
                <a href="{{ route('charge.chart.js', ['preset' => 'year']) }}" class="btn btn-success ml-5">هذه السنة</a>

                {{-- يدز زر الطباعة لليمين --}}
                <a href="{{ route('charge.chart.print', request()->query()) }}"
                    class="btn btn-link ml-auto"
                    title="طباعة">
                    <i class="mdi mdi-printer"></i>طباعة
                </a>
        </div>
        <form method="GET" id="rangeForm" action="{{ route('charge.chart.js') }}"  class="row g-2 mb-20 no-validate">
          ...
          <div id="feesDailyChart" style="min-height: 320px;"></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="row">
    <div class="col-12">
        ...
        <div class="col-md-6">
            <div class="box">
            <div class="box-header with-border">
                <h4 class="box-title">توزيع المصاريف حسب الفئة (%)</h4>
            </div>
            <div class="box-body">
                <div id="chargesDonut" style="min-height: 320px;"></div>
            </div>
            </div>
        </div>
        ...
        <div class="col-md-6">
            <div class="box">
            <div class="box-header with-border">
                <h4 class="box-title">توزيع الإيرادات حسب الفئة (%)</h4>
            </div>
            <div class="box-body">
                <div id="revenuDonut" style="min-height: 320px;"></div>
            </div>
            </div>
        </div>
        </div>
...
@push('scripts')
    ...
    if (typeof ApexCharts === "undefined") {
        ...
        return;
              ...
              const feesEl = document.querySelector("#feesDailyChart");
            if (feesEl) {
                const preset = @json($preset);
                const chartType = (preset === 'today') ? 'bar' : 'area';
                const feesLabels  = @json($feesLabels);
                const feesCharges = @json($feesCharges);
                const feesRevenu  = @json($feesRevenu);

                new ApexCharts(feesEl, {
                chart: { type: chartType, height: 320, toolbar: { show: true } },
                stroke: { curve: 'smooth', width: 2 },
                series: [
                    { name: 'مصاريف (Charges)', data: feesCharges, },
                    { name: 'إيرادات (Revenu)', data: feesRevenu },
                ],
                ...
                dataLabels: {
                  ...
                  legend: {
                        labels: { colors: '#ffffff' } // ✅ لون كتابة legend
                    },
                xaxis: {
                    categories: feesLabels,
                    title: {
                                text: 'التواريخ',
                ...
                stroke: { curve: 'smooth' },
                title: { text: 'المصاريف مقابل الإيرادات يوميًا', align: 'left', style: { color: '#ffffff' } },
                markers: { size: 4, colors: ['#FFA41B'], strokeColors: '#fff', strokeWidth: 2, hover: { size: 7 } },
                grid: { borderColor: '#f1f1f1' },



                    // labels: { formatter: v => v + ' DZD' } },
                tooltip: { y: { formatter: v => v + ' DZD' } }
                }).render();
            }

            // =====================
// ✅ Donut: Charges by category
// =====================
const chargesDonutEl = document.querySelector("#chargesDonut");
if (chargesDonutEl) {
  const chargesCatLabels = @json($chargesCatLabels ?? []);
  const chargesCatSeries = @json($chargesCatSeries ?? []);

  if (chargesCatSeries.length > 0) {
    new ApexCharts(chargesDonutEl, {
      chart: { type: 'donut', height: 320, toolbar: { show: false } },
      labels: chargesCatLabels,
      series: chargesCatSeries,
      dataLabels: {
        enabled: true,
      ...
      plotOptions: {
        pie: {
          donut: {
            labels: {
              show: true,
              name: { show: true, color: '#ffffff' },
              value: {
                show: true,
                color: '#ffffff',
                formatter: (v) => Number(v).toLocaleString('fr-FR') + ' DZD'
              },
              total: {
                show: true,
                ...
                formatter: function (w) {
                  ...
                  return sum.toLocaleString('fr-FR') + ' DZD';
                }
              }
            }
          }
    ...
    chargesDonutEl.innerHTML = '<div class="text-white">لا توجد مصاريف في هذه الفترة</div>';
  }
}

// =====================
// ✅ Donut: Revenu by category
// =====================
const revenuDonutEl = document.querySelector("#revenuDonut");
if (revenuDonutEl) {
  const revenuCatLabels = @json($revenuCatLabels ?? []);
  const revenuCatSeries = @json($revenuCatSeries ?? []);

  if (revenuCatSeries.length > 0) {
    new ApexCharts(revenuDonutEl, {
      chart: { type: 'donut', height: 320, toolbar: { show: false } },
      labels: revenuCatLabels,
      series: revenuCatSeries,
      dataLabels: {
        enabled: true,
        formatter: function (val) { return val.toFixed(1) + '%'; }
      },
    ...
    revenuDonutEl.innerHTML = '<div class="text-white">لا توجد إيرادات في هذه الفترة</div>';
  }
}
