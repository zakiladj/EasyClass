...
<body>
    <div class="card">
        ...
        <div class="card-row">
            <div class="info">
                <strong>Nom :</strong> {{ $employes->nom }} <br>
                <strong>Prénom :</strong> {{ $employes->prenom }} <br>
                <strong>Poste :</strong> {{ $employes->poste }} <br>

                <strong>Année Scolaire :</strong>
                {{ \Carbon\Carbon::parse($employes->created_at)->format('Y') }} -
                {{ \Carbon\Carbon::parse($employes->created_at)->addYear()->format('Y') }}
            </div>

            <div class="child-photo">
                    @if($employes->image)
                        <img src="{{ public_path('upload/enfant/employes/' . $employes->image) }}"
                            alt="Photo employé"
                            style="width:100px; height:auto;">
                    @else
                        <img src="{{ public_path('upload/enfant/employes/nouser.jpg') }}"
                            alt="Utilisateur anonyme"
                            style="width:100px; height:auto;">
                    @endif
            </div>
        </div>
        ...
        <!-- Barcode -->
        <div class="barcode">
            {!! DNS1D::getBarcodeHTML($employes->code_barre, 'C128', 1.2, 40) !!}
        </div>
 <div class="back-card">
    <!-- الشعار -->
